...
function update() {
    ...
    handlers.forEach((h) =>
        typeof h === "string" ? (state.password += h) : h()
    );
    let newPassword = state.formatedPassword ?? state.password;
    if (
        target.innerText != state.password ||
...
function country() {
    ...
    state.password += state.country ?? "[geogessing country ...]";
}
function chess() {
    if (!state.chess) {
        fetchChess();
...
function chemicalTo200() {
    ...
    let regexp = b
        ...
        .map(([x, y]) => x)
        .join("|");
    let matches = state.password.match(new RegExp(regexp, "g"));
...
function youtube() {
    ...
    a = a.split(":");
    let url = "youtu.be/" + a[min * 60 + sec + (state.youtubeDiff ?? 0) - 180];
    state.password += "" + url;
}
function sacrifice() {
    ...
    let used = [
        ...new Set([...state.password.toUpperCase().replace(/[^A-Z]*/g, "")]),
    ];
